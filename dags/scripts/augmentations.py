...
# Crossref API 
def fetch_article_augments(start_range, end_range, article):
    ...
    for i in range(start_range, end_range, 1): # don't use tqdm if range specified like that...
        try:
            ...
            print(f'There was somekind of error at request nr {i} - likely not a valid journal article or not found.')
            print(f'Continuing with next queries...')
            print()
            continue
...
# Check or prepare the journal metrics file
def check_or_prepare_journal_metrics():
    if os.path.exists('dags/augmentation/cwts2021.csv'):
        print("'cwts2021.csv exists!")
    else: 
        if os.path.exists('dags/augmentation/CWTS Journal Indicators April 2022.xlsx'):
            print("Preparing 'cwts2021.csv' from the .xslx source...")
            cwts_data = pd.read_excel('dags/augmentation/CWTS Journal Indicators April 2022.xlsx')
            # Fix colnames (replace spaces and lower)
            cwts_data.columns = [col.replace(' ','_').lower() for col in cwts_data.columns] 
            cwts2021 = cwts_data[cwts_data['year'].isin([2021])].reset_index(drop = True)
            cwts2021.to_csv('dags/augmentation/cwts2021.csv')
            print("'cwts2021.csv prepared and in the directory!")
        else:
            print("Downloading the .xslx source...")
            # Download the indicators
            url = 'https://www.journalindicators.com/Content/CWTS Journal Indicators April 2022.xlsx'
            path = 'dags/augmentation'
            wget.download(url, out = path)
            print(".xslx source downloaded!")
